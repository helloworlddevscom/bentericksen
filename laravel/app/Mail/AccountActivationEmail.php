 ...
 *
 * Mailable class for creating mailable object response for account activation events
 */
class AccountActivationEmail extends Mailable
{
    ...
    public function buildEntry()
    {
        ...
        if ($this->response == null) {
            $this->status = 'error';
            $this->error = 'Error - Logging error when trying to save email response code';
        }
        ...
        if ($this->user_id == null) {
            try {
                ...
                $this->status = 'Error';
                $this->error = 'Error - unable to find user - '.$exc->getCode().': '.$exc->getMessage();
            }
        }
    }
}
