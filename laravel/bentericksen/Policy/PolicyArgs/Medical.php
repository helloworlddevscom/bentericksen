...
class Medical
{
	...
	{
		$this->args['offered'] = $this->settings->medical->is_offered;
		$this->args['waiting_period'] = $this->settings->medical->waiting_period;
		...
		foreach( $this->settings->classifications as $key => $classification )
		{
			...
			foreach($classification->medical AS $ke => $val )
			{
