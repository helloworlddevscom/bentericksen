...
import { seedConfig } from '../../resources/assets/js/store/mock-data/101-02-bonusPro-wip-seed';

describe('Bonus Pro Plan Validation with Hygiene Plan', () => {
  it('Can generate plan setup and validate first month entry', () => {
    ...
    const templateParams = {
      init: {
        ...
        production_collection_salaries: "$53,017",
    ...
    function validateSnapshot (objDom, keys) {
        ...
        evalRow.forEach((data, index) => {
          ...
          if (key === 'production_amount' || key === 'collection_amount') {
            if (index > 0) {
              ...
              const result = dollarFormatter.format(activeSixMonth[index - 1][key]);
            ...
            if (index > 1) {
              ...
              if (activeSixMonth[index - 1][key] !== '-') {
                ...
                if (value.type === 'percentage') {
                  result = percentageFormatter.format(activeSixMonth[index - 1][key]);
                } else {
                  result = dollarFormatter.format(activeSixMonth[index - 1][key]);
                }
                const dataDom = data.$(`input.${value.id}`);
                expect(dataDom).toHaveValue(result);
              }
    ...
    validateSnapshot(staff, keys);

    // COMMENT BELOW to only generate a business/plan base.   No monthly data
    //
    browser.pause(2000)
    ...
    // TODO:  Add validation step for plan finished successfully (stored)
