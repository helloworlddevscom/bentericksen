			...
			<div class="row main_wrap">		
				...
				<!-- -->
					...
					<div class="col-md-12 content">
						{!! Form::open(['url' => ['/admin/job-descriptions', $jobDescription->id], 'class' => 'form-horizontal', 'method' => 'put']) !!}
							<div class="form-group">
								<label for="job_description" class="col-md-3 col-md-offset-2 control-label">Job Description:</label>
								<div class="col-md-3">
									<input type="text" class="form-control" id="job_description" name="name" placeholder="Job Description" value="{{ $jobDescription->name }}">
								</div>
							</div>
							...
							<div class="form-group">
								<div class="col-md-10 col-md-offset-1">
									<textarea class="form-control ckeditor" name="description">{{ $jobDescription->description }}</textarea>
								</div>
							</div>
...
@section('foot')
	...
	$(document).ready(function() {
		...
		subs = $.parseJSON('{!!$jobDescription->subtype!!}');
		$('#job_industry option[value="{{ $jobDescription->industry }}"]').prop('selected', true);
		$.each(subs, function(index, value) {
			$('#job_sub_type option[value="'+value+'"]').prop('selected', true);
		});
		$('#job_sub_type option.{{ $jobDescription->industry }}').each(function() {
			$(this).removeClass('hidden');
		});				
	});
