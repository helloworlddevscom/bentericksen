			...
			<div class="row main_wrap">		
				...
				<!-- -->
					<div class="col-md-12 heading">
						<h3><a href="/user/job-description">Forms</a> / Edit</h3>
					</div>
					
					<div class="col-md-12 content">
						{!! Form::open(['url' => ['/user/forms', $form->id], 'class' => 'form-horizontal', 'method' => 'put']) !!}
							<div class="form-group">
								<label for="job_description" class="col-md-3 col-md-offset-2 control-label">Form:</label>
								<div class="col-md-3">
									<input type="text" class="form-control" id="job_description" name="name" placeholder="Form Title" value="{{ $form->name }}">
								</div>
							</div>
							...
							<div class="form-group">
								<div class="col-md-10 col-md-offset-1">
									<textarea class="form-control ckeditor" name="description">{{ $form->description }}</textarea>
								</div>
							</div>
...
@section('foot')
	...
	$(document).ready(function() {
		...
		subs = $.parseJSON('{!!$form->subtype!!}');
		$('#job_industry option[value="{{ $form->industry }}"]').prop('selected', true);
		$.each(subs, function(index, value) {
			$('#job_sub_type option[value="'+value+'"]').prop('selected', true);
		});
		$('#job_sub_type option.{{ $form->industry }}').each(function() {
			$(this).removeClass('hidden');
		});		
