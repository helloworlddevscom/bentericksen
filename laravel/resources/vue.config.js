 ...
 * @returns 
 */
const page = (entry = 'main', name, outputPath, template = 'app') => {
  return {
    [name]: {
      entry: `src/entrypoints/${entry}.js`,
      filename: path.resolve(__dirname, `views/${environmentViewsDirectory}${outputPath}.blade.php`),
      template: `src/templates/${template}.blade.php`
    }
  }
}
...
module.exports = {
  ...
  pages: {
    ...page('main', 'app', 'app'),
    ...page('auth', 'auth', 'auth/wrap', 'auth'),
    ...page('admin', 'admin', 'admin/wrap', 'admin'),
    ...page('main', 'admin-inertia', 'admin/wrap-inertia', 'admin-inertia'),
    ...page('user', 'user', 'user/wrap', 'user'),
    ...page('employee', 'employee', 'employee/wrap', 'employee'),
    ...page('consultant', 'consultant', 'consultant/wrap', 'consultant'),
  },
  // Where to dump resulting files
  outputDir: `../../httpdocs/${outputDir}`,
