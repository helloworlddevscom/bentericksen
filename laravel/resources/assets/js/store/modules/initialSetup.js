...
const actions = {
  saveDraft: ({ commit, rootState }) => {
    commit('update', { prop: 'business_id', value: rootState.global.businessId });
    commit('update', { prop: 'created_by', value: rootState.global.createdBy });
  ...
  save: ({ commit, dispatch, state, rootState }) => {
    ...
    return new Promise((resolve, reject) => {
      BonusProService.save(state, 'initialSetup')
        .then((data) => {
          commit('setInitialSetupData', data.plan);
          if (rootState.planData.months.length === 0) {
            dispatch('planData/initializeMonths', {}, { root: true });
          }
          resolve({ success: true });
        })
        .catch((err) => {
  ...
  setInitialSetupData: ({ commit, dispatch, rootState }, payload) => {
    ...
    if (!payload) {
     return dispatch('ui/updateStep', 'initialSetup', { root: true });
    }
    ...
    if (payload.months.length > 0) {
      dispatch('planData/setMonths', payload.months, { root: true });
    } else {
      dispatch('planData/initializeMonths', {}, { root: true });
    }

    if (payload.users && payload.users.length > 0) {
      dispatch('employeesAndFunds/setEmployees', payload.users, { root: true });
    }

    if (payload.funds && payload.funds.length > 0) {
      dispatch('employeesAndFunds/setFunds', payload.funds, { root: true });
    }

    dispatch('ui/updateStep', payload.current_step || defaultCurrentStep, {
      root: true
    });
