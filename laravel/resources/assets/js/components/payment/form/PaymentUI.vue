...
export default {
  ...
  computed: {
    ...
    ...mapState({
      'manageCard': state => state.manageCard,
      'submitButtonLabel': state => state.submitButtonLabel,
      'editCard': state => state.editCard,
      'existingCardInfo': state => {
        const info = state.existingCardInfo;
        for (const key in info) {
          if (info[key].toString().length < 1) {
            return true;
          }
        }
        if (!state.chargeNowOption.length && !state.chargeLaterOption.length) {
          return true
        }
        return false;
      },
      'newCardInfo': state => {
        const info = state.newCardInfo;
        for (const key in info) {
          if (key === 'line2') {
            continue;
          }
  ...
  methods: {
    editCardAction (value) {
      this.$store.dispatch('editCardAction', value)
    },
    submitCC () {
      this.$emit('submitCC', this.$store.state.submitButtonAction);
    },
    showManageCardModalAction (action) {
      this.$store.dispatch('showManageCardModalAction', action);
    }
  }
}
</script>
