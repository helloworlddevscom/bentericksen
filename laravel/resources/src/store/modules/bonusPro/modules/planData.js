...
import { compareMonths } from '@/store/modules/bonusPro/utils'
import { month } from '@/store/modules/bonusPro/schemas'
...
export const getters = {
  activeMonths: (state, getters, rootState) => {
    ...
    return state.months
      .filter(mon => mon.finalized || !rootState.bonusPro.initialSetup.completed)
  ...
  activeMonthHoursTotal (state, getters, rootState, rootGetters) {
    return (key = 'staffHoursTotal') => {
      if (rootGetters['bonusPro/initialSetup/hygienePlan']) {
  ...
  employeeHoursTotal (state, getters, rootState, rootGetters) {
    return (employees) => employees
      .reduce((out, {monthEmployee, employee}) => {
        if (employee.bp_employee_type === 'hygienist' && rootGetters['bonusPro/initialSetup/hygienePlan']) {
...
export const actions = {
  ...
  setMonths: ({ commit, dispatch, rootState }, months) => {
    const mo = month();
    const rollingAverage = rootState.bonusPro.initialSetup.rolling_average;
  ...
  initializeMonths: ({ commit, rootState }) => {
    const startMonth = rootState.bonusPro.initialSetup.start_month;
    const startYear = rootState.bonusPro.initialSetup.start_year;
    const rollingAverage = rootState.bonusPro.initialSetup.rolling_average;
    let k = 0;
    let date;
  ...
  calculateBonusesToBePaid: ({ commit, rootState }, month) => {
    const staffBonusPercentage = parseFloat(rootState.bonusPro.initialSetup.staff_bonus_percentage);
    const hygieneBonusPercentage = parseFloat(rootState.bonusPro.initialSetup.hygiene_bonus_percentage);
    ...
    rootState.bonusPro.employeesAndFunds.funds.forEach((fund) => {
  ...
  calculateHoursTotals: ({ commit, rootState, rootGetters }, month) => {
    ...
    rootGetters['bonusPro/employeesAndFunds/allEmployees'](month).forEach(({monthEmployee, employee}) => {
      if (employee.bp_employee_type === 'hygienist' && rootGetters['bonusPro/initialSetup/hygienePlan']) {
  ...
  calculateSalaryBonusTotals: ({ commit, rootState, rootGetters }, month) => {
    let staffTotal = 0
    let hygienistsTotal = 0
    const hasHygiene = rootState.bonusPro.initialSetup.hygiene_plan
  ...
  calculateFundsBonusAmount: ({ commit, state, rootState }, month) => {
    ...
    fundsCopy.sort(function (a, b) {
      const fundA = rootState.bonusPro.employeesAndFunds.funds.find((el) => {
        return el.id === a.fund_id;
      });
      const fundB = rootState.bonusPro.employeesAndFunds.funds.find((el) => {
    ...
    fundsCopy.forEach((fund) => {
      const fundData = rootState.bonusPro.employeesAndFunds.funds.find((el) => {
  ...
  calculateEmployeeBonusAmount: ({ commit, state, rootState, rootGetters }, month) => {
    ...
    rootGetters['bonusPro/employeesAndFunds/allEmployees'](month).forEach(({monthEmployee, employee, index}) => {
  ...
  calculateEmployeeBonusPercentage: ({ commit, state, rootState, getters, rootGetters }, month) => {
    ...
    const equalShareDistType = rootState.bonusPro.initialSetup.distribution_type == 'equal_share'
    const eligibleEmployees = rootGetters['bonusPro/employeesAndFunds/eligibleEmployees'](month)
    const numberOfHygienistsInPlan = equalShareDistType && rootGetters['bonusPro/initialSetup/hygienePlan'] && eligibleEmployees
      .filter(({employee}) => employee.bp_employee_type === 'hygienist')
      .length || 0
    const numberOfStaffInPlan = equalShareDistType && eligibleEmployees
      .filter(({employee}) => employee.bp_employee_type !== (rootGetters['bonusPro/initialSetup/hygienePlan'] ? 'hygienist' : false))
      .length || 0
    ...
    rootGetters['bonusPro/employeesAndFunds/ineligibleEmployees'](month).forEach(({ index }) => {
    ...
    eligibleEmployees.forEach(({monthEmployee, employee, index}) => {
      ...
      switch (rootState.bonusPro.initialSetup.distribution_type) {
        case 'equal_share':
          if (employee.bp_employee_type === 'hygienist' && rootGetters['bonusPro/initialSetup/hygienePlan']) {
  ...
  updateProductionCollectionAverages: ({ commit, dispatch, rootState }) => {
    const months = rootState.bonusPro.planData.months;
    const rollingAverage = parseFloat(rootState.bonusPro.initialSetup.rolling_average);
