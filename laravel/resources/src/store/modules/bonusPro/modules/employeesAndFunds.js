...
import { compareMonths, employeeEligibleForBonus } from '@/store/modules/bonusPro/utils'
import { monthlyData, fundData, employeeFields } from '@/store/modules/bonusPro/schemas'
  ...
  activeMonthlyData: (state, getters, rootState) => {
    ...
    return state.activeEmployee.monthlyData
      .filter(mon => {
        ...
        const plan_month = rootState.bonusPro.planData.months.find(m => m.id === mon.month_id);
...
const actions = {
  saveMonthData: ({ commit, dispatch, rootState }, monthId) => {
    ...
    rootState.bonusPro.employeesAndFunds.employees.forEach((el) => {
    ...
    rootState.bonusPro.employeesAndFunds.funds.forEach((el) => {
      const month = el.monthlyData.find((m) => {
        return m.month_id === monthId;
      });
    ...
    obj.planId = rootState.bonusPro.initialSetup.id;
    obj.monthlyData = monthlyData;
    obj.fundsData = fundsData;
    obj.activeMonthData = {
      monthId: rootState.bonusPro.planData.activeMonth.id,
      hygiene_production_amount: rootState.bonusPro.planData.activeMonth.hygiene_production_amount,
      production_amount: rootState.bonusPro.planData.activeMonth.production_amount,
      collection_amount: rootState.bonusPro.planData.activeMonth.collection_amount,
      production_collection_average: rootState.bonusPro.planData.activeMonth.productionCollectionAverage,
      staff_percentage: rootState.bonusPro.initialSetup.staff_bonus_percentage,
      hygiene_percentage: rootState.bonusPro.initialSetup.hygiene_bonus_percentage
    };
  ...
  parseMonthlyData: ({ commit, state, dispatch }, { monthlyData, fundsData, newMonth }) => {
    dispatch('bonusPro/planData/addNewMonth', newMonth, { root: true });
  ...
  createEmployee: ({ state, commit, dispatch, rootState }) => {
    const employee = Object.assign({}, rootState.bonusPro.employeesAndFunds.activeEmployee);
    ...
    employee.plan_id = rootState.bonusPro.initialSetup.id;
    employee.distribution_type = rootState.bonusPro.initialSetup.distribution_type;
    ...
    return new Promise((resolve, reject) => {
      BonusProService.save(employee, 'createEmployee')
        .then((data) => {
          ...
          dispatch('bonusPro/global/addUserToBusiness', data.employeeData, { root: true });
          dispatch('setEmployeesMonthlyData');
  ...
  removeEmployee: ({ commit, dispatch, rootState }, id) => {
    return new Promise((resolve, reject) => {
      BonusProService.delete({ id: id, plan_id: rootState.bonusPro.initialSetup.id }, 'removeEmployee')
        .then(() => {
  ...
  updateFund: ({ commit, dispatch, state, rootState }, updateFundData) => {
    updateFundData.plan_id = rootState.bonusPro.initialSetup.id;
    ...
    return new Promise((resolve, reject) => {
      BonusProService.save(updateFundData, 'updateFund')
        .then((data) => {
          ...
          fund.monthlyData = [];
          rootState.bonusPro.planData.months.forEach((el) => {
  ...
  addFund: ({ commit, dispatch, state, rootState }, newFundData) => {
    newFundData.plan_id = rootState.bonusPro.initialSetup.id;
    commit('clearErrors', 'funds');
    return new Promise((resolve, reject) => {
  ...
  setFunds: ({ commit, rootState }, funds) => {
    const months = rootState.bonusPro.planData.months;
    funds.map((fund) => {
      fund.monthlyData = [];
      months.forEach((el) => {
        const data = el.funds.find((fnd) => {
  ...
  setEmployeesMonthlyData: ({ state, commit, rootState, dispatch }) => {
    if (rootState.bonusPro.ui.currentStep === 'employees') {
      // initialization
      const months = rootState.bonusPro.planData.months;
      months.forEach((month) => {
        ...
        dispatch('bonusPro/planData/setEmployeeMonthlyData', { month: month, monthlyData: employeeMonthly }, { root: true });
      });
      dispatch('bonusPro/planData/runCalculations', null, { root: true });
    } else {
      const month = rootState.bonusPro.planData.months[rootState.bonusPro.planData.months.length - 1];
      const employeeMonthly = [];
      state.employees.map((employee) => {
        const empData = employee.monthlyData.find((el) => {
          return el.month_id === month.id
  ...
  setFundsMonthlyData: ({ commit, rootState, dispatch }) => {
    const month = rootState.bonusPro.planData.months[rootState.bonusPro.planData.months.length - 1];
    const fundsMonthly = [];
    rootState.bonusPro.employeesAndFunds.funds.map((funds) => {
      const empData = funds.monthlyData.find((el) => {
        return el.month_id === month.id
      });
      fundsMonthly.push(empData);
    });
    dispatch('bonusPro/planData/setFundsMonthlyData', { month: month, monthlyData: fundsMonthly }, { root: true });
  },
  ...
  setActiveEmployee: ({ commit, rootState }, id) => {
    ...
    if (id) {
      employee = rootState.bonusPro.employeesAndFunds.employees.find((el) => {
        return el.id === id;
      });

      if (!employee) {
        employee = rootState.bonusPro.global.businessUsers.find((el) => {
          return el.id === id;
        });
      ...
      employee.business_id = rootState.bonusPro.global.businessId;
      months.forEach((el) => {
        employee.monthlyData.push(monthlyData(employee.id, el.id, el.month, el.year));
      });
    }
