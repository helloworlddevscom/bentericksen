...
export default {
  ...
  computed: {
    ignoreState () {
      return this.$store.getters['payments/ignoreState'];
    }
  },
  methods: {
    optionSelected (event) {
      this.$store.dispatch('payments/chargeNowOptionSelected', event.target.value);
      this.$store.dispatch('payments/submitButtonUpdate', event.target.value);
    }
  }
}
