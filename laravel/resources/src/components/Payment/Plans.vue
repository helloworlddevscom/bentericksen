...
export default {
  ...
  methods: {
    updateSelection () {
      this.$store.dispatch('payments/planOptionSelected', this.selection);
    },
    setSelection () {
    ...
    appliedDiscount () {
      if (this.$store.state.payments.chargeLater || (this.$store.state.payments.chargeNow && this.$store.state.payments.chargeNowOption !== "autopay") || !Object.keys(this.customerDiscount).length) {
        return "None";
      }
