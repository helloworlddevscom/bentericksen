...
export default {
  ...
  methods: {
    optionSelected (event) {
      ...
      this.$store.dispatch('payments/chargeOptionUpdated', event.target.value);
    },
    chargeNowVisibilityChanged (isVisible) {
      if (isVisible) {
        EventBus.$emit('chargeLaterOptionDeselected');
        this.$store.dispatch('payments/submitButtonUpdate', 'submit');
      }
    },
    chargeLaterVisibilityChanged (isVisible) {
      if (isVisible) {
        EventBus.$emit('chargeNowOptionDeselected');
        this.$store.dispatch('payments/chargeLaterOptionSelectUI', true);
        this.$store.dispatch('payments/submitButtonUpdate', 'save_only');
      }
    }
  }
}
