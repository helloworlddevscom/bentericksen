...
export default {
  ...
  computed: {
    ...
    chargeLater() {
      return this.$store.state.payments.chargeLater;
    },
    chargeLaterOption() {
      return this.$store.state.payments.chargeLaterOption;
    },
    chargeNow() {
      return this.$store.state.payments.chargeNow;
    },
    chargeNowOption() {
      return this.$store.state.payments.chargeNowOption;
    },
    showManageCardModalAction () {
      return this.$store.state.payments.showManageCardModalAction;
    },
    displayManageCardModal () {
      return this.$store.state.payments.displayManageCardModal;
    },
    paymentType () {
      return this.$store.state.payments.paymentType;
    },
    savePaymentMode () {
      return this.$store.state.payments.savePaymentMode;
    },
    makeDefaultPayment () {
      return this.$store.state.payments.makeDefaultPayment;
    },
    manageCard () {
      return this.$store.state.payments.manageCard;
    },
    replaceMode () {
      return this.$store.state.payments.replaceMode;
    },
    existingCardInfo() {
      let cardSelection = this.$store.state.payments.existingCardInfo;
    ...
    newCardInfo() {
      let creditCard = this.$store.state.payments.newCardInfo;
    ...
    planOptionSelection() {
      let selection = this.$store.state.payments.planOptionSelection;
    ...
    lockSubmitACH () {
      return this.$store.state.payments.paymentType === '' || this.productID === '';
    },
    lockSubmitVerifyACH () {
  ...
  methods: {
    ...
    */
    configureStripe() {
      this.stripe = Stripe(this.$store.state.config.stripe_pk);
    ...
    */
    ...
    async postTransaction(url, dataToken, el) {
      ...
      if (
        ...
        this.response.transaction.data === "Payment instrument successfully updated." && this.$store.state.payments.chargeLaterOption === "save_only" ||
        this.response.transaction.data === "Subscription purchase successful." &&
        (this.$store.state.payments.chargeNowOption === "autopay" || this.$store.state.payments.chargeNowOption === "onetime_save" || this.$store.state.payments.chargeNowOption === "onetime_ignore")) {
        setTimeout(() => {
      ...
      // Set UI state
      await this.$store.dispatch('payments/replaceMode', false);
      await this.$store.dispatch('payments/editCardAction', false);

    },
    applyPaymentEntryState() {
      if (this.response.setup.data.cards.length) {
        this.$store.dispatch('payments/manageCard', true);
    ...
    getInstrumentMode() {
      ...
      if (this.cardID === '' || this.replaceMode) {
        if (this.$store.state.payments.savePaymentMode === 'save') {
          return 'save';
        } else {
          return 'ignore';
        }
    ...
    chargeNowOptionDeselected() {
      this.$store.dispatch('payments/paymentType', '');
      this.$store.dispatch('payments/savePaymentMode','');
    },
    chargeLaterOptionDeselected() {
      this.$store.state.payments.chargeLaterOption = "";
    },
    submitCC(type) {
      switch (type) {
        case 'addCC':
    ...
    */
    ...
    async updateCC() {
      ...
      if (currentCard.name === this.fullName &&
        currentCard.exp_month === this.exp_month
        && currentCard.exp_year === this.exp_year) {
      ...
      const dataToken = {
        ...
        default_instrument: this.$store.state.payments.makeDefaultPayment,
    ...
    async purchaseCC() {
      ...
      if(this.replaceMode && this.$store.state.payments.chargeNowOption !== "onetime_ignore") {
        this.deleteCC();
      }
      ...
      // for existing payments, we send back custom ID as token
      if (this.$store.state.payments.manageCard) {
        token = this.cardID;
      } else {
    ...
    processCCTransaction (transactionToken, el) {
      const dataToken = {
        ...
        payment_type: this.$store.state.payments.paymentType,
        default_instrument: this.$store.state.payments.makeDefaultPayment,
